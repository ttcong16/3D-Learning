...
def get_pixels_from_image(image_size, camera):
    ...
    xy_grid = torch.stack(
        tuple( reversed( torch.meshgrid(y, x, indexing='ij') ) ),
        dim=-1,
    ).view(W * H, 2)
...
def get_random_pixels_from_image(n_pixels, image_size, camera):
    ...
    num_total = xy_grid.shape[0]
    n = min(n_pixels, num_total)
    rand_idx = torch.randperm(num_total, device=device)[:n_pixels]
    xy_grid_sub = xy_grid[rand_idx]
...
def get_rays_from_pixels(xy_grid, image_size, camera):
    ...
    if world_points.dim() == 3:
        B, N, _ = world_points.shape
        world_points = world_points.reshape(B * N, 3)
        camera_center = camera.get_camera_center().reshape(B, 1, 3).expand(B, N, 3).reshape(B * N, 3)
    else:
        N = world_points.shape[0]
        camera_center = camera.get_camera_center().view(1, 3).expand(N, 3)  # (N, 3)
    ...
    rays_d = F.normalize(world_points - rays_o, dim=-1)  # (N, 3)
