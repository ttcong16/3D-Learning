...
def evaluate(predictions, mesh_gt, thresholds, args):
    if args.type == "vox":
        ...
        print("num of occupied voxels:", (voxels_src > 0.1).sum())
        vertices_src, faces_src = mcubes.marching_cubes(voxels_src.detach().cpu().squeeze().numpy(), isovalue=0.1)
        ...
        # 1) marching cubes -> vertices ở không gian chỉ số voxel [0..R-1]
        v_src, f_src = mcubes.marching_cubes(
            vol.detach().cpu().squeeze().numpy(), 0.5  # iso = 0.5
        )
        if v_src.size == 0:
            pred_points = torch.zeros(B, args.n_points, 3)
        else:
            v_src = (v_src / (R - 1)) * 2.0 - 1.0
            v_src = torch.tensor(v_src, dtype=torch.float32)
            f_src = torch.tensor(f_src.astype(int))
            mesh_src = pytorch3d.structures.Meshes([v_src], [f_src])
            pred_points = sample_points_from_meshes(mesh_src, args.n_points)  # (1,n,3)
            pred_points = pred_points.expand(B, -1, -1).contiguous()          # (B,n,3)
            pred_points = pred_points - pred_points.mean(1, keepdim=True)
